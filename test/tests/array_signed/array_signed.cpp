...
public:
    static ProtoType encoderInit(const EncoderContext& ctx) {
        return ProtoType{
                .values = ArraySignedConverter<LOCAL_TestMessage::ContainerType>::encoderInit(ctx.v.values)
        };
    }

    static ProtoType decoderInit(DecoderContext& ctx){
        return ProtoType{
                .values = ArraySignedConverter<LOCAL_TestMessage::ContainerType>::decoderInit(ctx.v.values)
        };
    }
