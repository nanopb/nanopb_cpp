...
public:
    static ProtoType encoderInit(const EncoderContext& ctx) {
        return ProtoType{
                .values = ArrayStringConverter<LOCAL_TestMessage::ContainerType>::encoder(ctx.local.values)
        };
    }

    static ProtoType decoderInit(DecoderContext& ctx){
        return ProtoType{
                .values = ArrayStringConverter<LOCAL_TestMessage::ContainerType>::decoder(ctx.local.values)
        };
    }
