...
public:
    static ProtoType encoderInit(const LocalType& local) {
        return ProtoType{
                .values = ArrayDoubleCallbackConverter<typename LocalType::ContainerType>::encoderCallbackInit(local.values)
        };
    }

    static ProtoType decoderInit(LocalType& local){
        return ProtoType{
                .values = ArrayDoubleCallbackConverter<typename LocalType::ContainerType>::decoderCallbackInit(local.values)
        };
    }
...
int testRepeated(){
    ...
    LOCAL_TestMessage<CONTAINER> original({-(float)(INT64_MAX), -100.45f, 0, 1.3f , 212.f, 544.4f, (float)INT64_MAX});
