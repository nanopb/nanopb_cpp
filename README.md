...
* `NanoPb::ScalarConverter` - Basic scalar type like `enum`.
* `NanoPb::MessageConverter` - Converter for messages.
* `NanoPb::UnionMessageConverter` - Converter for union (oneof) messages.
* `NanoPb::CallbackConverter` - Converter for callbacks.
* `NanoPb::StringCallbackConverter` - Converter for variable size string, mapped to `std::string`.
* `NanoPb::RepeatedCallbackConverter` - Converter for repeated message.
* `NanoPb::ArrayUnsignedCallbackConverter` - Repeated convertor for `std::vector<unsigned>` or `std::list<unsigned>` (Also support other signed types: `uint64_t`, `uint32_t`, etc).
* `NanoPb::ArraySignedCallbackConverter` - Repeated convertor for `std::vector<int>` or `std::list<int>` (Also support other signed types: `int64_t`, `int32_t`, etc).
* `NanoPb::ArrayStringCallbackConverter` - Repeated convertor for `std::vector<std::string>` or `std::list<std::string>`.
* `NanoPb::ArrayMessageCallbackConverter` - Repeated convertor for `std::vector<CLASS>` or `std::list<CLASS>`, NOTE: `MessageConverter` for  `CLASS` should be defined. 
* `NanoPb::MapCallbackConverter` - Map with any type of the key and value.
